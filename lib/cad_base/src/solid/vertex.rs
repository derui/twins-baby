use std::ops::Deref;

use crate::vector3::Vector3;

/// Immutable vertex for solid.
#[derive(Debug, Clone, PartialEq)]
pub struct Vertex(Vector3);

impl Vertex {
    /// Get new vertex
    pub fn new(vec: &Vector3) -> Self {
        Vertex(*vec)
    }
}

// Default must be origin point.
impl Default for Vertex {
    fn default() -> Self {
        Vertex::new(&Vector3::default())
    }
}

impl From<Vector3> for Vertex {
    fn from(vec: Vector3) -> Self {
        Vertex(vec)
    }
}

impl Deref for Vertex {
    type Target = Vector3;

    fn deref(&self) -> &Self::Target {
        &self.0
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use approx::assert_relative_eq;
    use pretty_assertions::assert_eq;
    use rstest::rstest;

    #[rstest]
    #[case(0.0, 0.0, 0.0)]
    #[case(1.0, 2.0, 3.0)]
    #[case(-1.5, 0.0, 99.9)]
    fn new_creates_vertex_with_given_coordinates(#[case] x: f32, #[case] y: f32, #[case] z: f32) {
        // Act
        let v = Vertex::new(&Vector3::new(x, y, z));

        // Assert
        assert_relative_eq!(v.x, x);
        assert_relative_eq!(v.y, y);
        assert_relative_eq!(v.z, z);
    }

    #[test]
    fn default_is_origin() {
        // Act
        let v = Vertex::default();

        // Assert
        assert_relative_eq!(v.x, 0.0);
        assert_relative_eq!(v.y, 0.0);
        assert_relative_eq!(v.z, 0.0);
    }

    #[test]
    fn equality_for_same_coordinates() {
        // Arrange
        let v1 = Vertex::new(&Vector3::new(1.0, 2.0, 3.0));
        let v2 = Vertex::new(&Vector3::new(1.0, 2.0, 3.0));

        // Assert
        assert_eq!(v1, v2);
    }

    #[test]
    fn inequality_for_different_coordinates() {
        // Arrange
        let v1 = Vertex::new(&Vector3::new(1.0, 2.0, 3.0));
        let v2 = Vertex::new(&Vector3::new(1.0, 2.0, 4.0));

        // Assert
        assert_ne!(v1, v2);
    }

    #[test]
    fn clone_produces_equal_vertex() {
        // Arrange
        let v1: Vertex = Vector3::new(1.0, 2.0, 3.0).into();

        // Act
        let v2 = v1.clone();

        // Assert
        assert_eq!(v1, v2);
    }
}
