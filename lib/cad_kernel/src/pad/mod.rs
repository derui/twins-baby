...
use cad_base::{
    feature::{
        AttachedTarget, Evaluate, EvaluateError, Feature, FeatureContext, operation::{Operation, Pad, PadDirection}
    }, id::{EdgeId, VertexId}, plane::Plane, point::Point, solid::{Solid, SolidBuilder, edge::Edge, face::{Face, PlanarSurface}, vertex::Vertex}, vector3::Vector3
};
...
fn compute_moved_face<'a>(
    ...
    let moved_points: Vec<_> = curve.points.iter().map(|p| {
    ...
    let edges: Vec<_> = curve.edges.iter().filter_map(|(start, end)| {
        let Some(start) = vertex_ids.get(*start) else {return None;};
        let Some(end) = vertex_ids.get(*end) else {return None;};
        
        Some(Edge::new(*start, *end).expect("Must be success"))
    }).collect();
    
    assert!(edges.len() == curve.edges.len(), "Must keep same number of edges from sketch");
        
    let edge_ids = builder.add_edges(&edges);
    
    builder.add_faces(&[Face::Planar(PlanarSurface::new(&edge_ids, plane).expect("should be valid"))]);

    (vertex_ids, edge_ids)
}

/// Compute faces surrounding of the solid
fn compute_surrounding_faces(builder: &mut SolidBuilder, first: &(Vec<VertexId>, Vec<EdgeId>), second: &(Vec<VertexId>, Vec<EdgeId>)) {
    let (_, f_edge) = first; 
    let (_, s_edge) = second;

    assert!(f_edge.len() == s_edge.len(), "Must same first and second face's edges {} <> {}", f_edge.len(), s_edge.len());
    ...
    for idx in 0..f_edge.len() {
        ...
        let new_edge_f = builder.get_edge_by_pair(&*f_edge.start, &*s_edge.start).unwrap_or_else(|| {
            let e = Edge::new(*f_edge.start, *s_edge.start).expect("Must be success");
            builder.add_edges(&[e])[0]
        });

        let new_edge_e = builder.get_edge_by_pair(&*f_edge.end, &*s_edge.end).unwrap_or_else(|| {
            let e = Edge::new(*f_edge.end, *s_edge.end).expect("Must be success");
            builder.add_edges(&[e])[0]
        });

        // compute normal vector of the face
        let edge1 = (&**builder.get_vertex(&f_edge.start).expect("Must be exist"), &**builder.get_vertex(&f_edge.end).expect("Must be exist"));
        let edge2 = (&**builder.get_vertex(&f_edge.start).expect("Must be exist"), &**builder.get_vertex(&s_edge.start).expect("Must be exist"));
        
        let plane = Plane::<DefaultEpsilon>::new(edge1, edge2).expect("This plane must be creatable");
        let face = Face::Planar(PlanarSurface::new(&[f_edge_id, s_edge_id, new_edge_f, new_edge_e], &plane).expect("This face must be creatable"));
...
fn compute_pad<'a>(
    ...
    let sketcher = Sketcher::new(&*context.sketches[0], &context.target[0]).map_err(|e| {
        EvaluateError::HaveSomeInvalidSketches(e.into())
    })?;
    let curves = sketcher.calculate_jordan_corves::<DefaultEpsilon>().map_err(|e| EvaluateError::HaveSomeInvalidSketches(e.into()))?;

    let plane = match context.target[0] {
        AttachedTarget::Plane(plane) => plane,
        AttachedTarget::Face(face) => todo!(),
    };
    ...
    let length = (*pad.size).evaluate(&Environment::empty()).expect("This equation must not to use variable now");
    for curve in &curves {
        ...
        let first_planes;
        {
            ...
            first_planes = compute_moved_face(&mut solid, curve, first_plane.as_ref().unwrap_or(plane), length);
        }
