...
impl Vector {
    ...
    pub fn from(vec: &[f32]) -> Result<Self, color_eyre::eyre::Error> {
        if vec.is_empty() {
            return Err(color_eyre::eyre::eyre!("Can not define 0-dimension vector"));
        }
    ...
    pub fn zero(size: usize) -> Result<Self, color_eyre::eyre::Error> {
        if size == 0 {
            return Err(color_eyre::eyre::eyre!("Can not define 0-dimension vector"));
        }
...
impl Add<Vector> for Vector {
    type Output = Result<Vector, color_eyre::eyre::Error>;
...
mod tests {
    ...
    #[case(1)]
    #[case(5)]
    fn test_zero_creates_zero_vector(#[case] size: usize) -> Result<(), color_eyre::eyre::Error> {
        // Arrange & Act
    ...
    #[test]
    fn test_add_returns_error_for_different_dimensions() -> Result<(), color_eyre::eyre::Error> {
        // Arrange
    ...
    #[test]
    fn test_sub_returns_error_for_different_dimensions() -> Result<(), color_eyre::eyre::Error> {
        // Arrange
        let vector1 = Vector::from(&[1.0, 2.0, 3.0])?;
        let vector2 = Vector::from(&[4.0, 5.0])?;
